...
// Quick CloudKit Migration
// Add this to your app to run the migration

class QuickCloudKitMigration: ObservableObject {
    private let container = CKContainer(identifier: "iCloud.com.cheffy.app")
    private let database: CKDatabase
    
    @Published var isRunning = false
    @Published var progress: Double = 0.0
    @Published var currentStatus = "Ready to start"
    @Published var totalRecipes = 0
    @Published var processedRecipes = 0
    @Published var successfulUploads = 0
    @Published var failedUploads = 0
    ...
    func startMigration() async {
        ...
        do {
            // Load all recipes
            await MainActor.run { currentStatus = "Loading recipes from JSON files..." }
            let allRecipes = try await loadAllRecipesFromJSON()
            
            await MainActor.run {
                totalRecipes = allRecipes.count
                currentStatus = "Loaded \(allRecipes.count) recipes. Starting upload..."
            }
        ...
        for (cuisineName, cuisineRecipes) in cuisines {
            for recipeData in cuisineRecipes {
                do {
                    ...
                    print("Failed to convert recipe: \(error)")
                }
            }
        }
    ...
    private func uploadRecipesInBatches(_ recipes: [CloudKitRecipe]) async throws {
        ...
        for i in stride(from: 0, to: recipes.count, by: batchSize) {
            ...
            await MainActor.run {
                currentStatus = "Uploading batch \(batchNumber)/\(totalBatches)..."
            }
            ...
            await MainActor.run {
                ...
                progress = Double(processedRecipes) / Double(recipes.count)
            }
